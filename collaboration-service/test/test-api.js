...
describe('Collaboration Service API', () => {
  ...
  describe('#deleteRoom', () => {
    ...
    it('missing room', async function () {
      ...
      assert.equal(
        result.body.message,
        'Unable to query: Error: Room ID is not of type string'
      );
    });
    ...
    it('invalid room', async function () {
      ...
      assert.equal(result.body.message, 'No room found');
    });
  ...
  describe('#getRoomDetails', () => {
    ...
    it('invalid roomId', async function () {
      const result = await request(app)
        .get('/api/collaboration-service/rooms?roomId=asdnasdsa')
        .expect(404)
        .expect('Content-Type', /json/);
    ...
    it('valid roomId', async function () {
      ...
      const result = await request(app)
        .get(
          `/api/collaboration-service/rooms?roomId=${create.body.data.roomId}`
        )
        .expect(200)
        .expect('Content-Type', /json/);
